...
const allThumbnails = computed(() => {
  ...
  // 僅顯示已收藏的 Thumbnail
  filtered = filtered.filter((t) => favoriteThumbnailIds.value.includes(t.id))
...
function handleToggleFavorite(thumbnailId: string) {
  const favoriteSet = new Set(favoriteThumbnailIds.value)
  ...
  favoriteThumbnailIds.value = Array.from(favoriteSet)
}
